...
class Testคำบอกวันที่ที่อยู่ใกล้กว่าต้องชนะบริบทที่อยู่:
    """ยามบริบทที่อยู่มองย้อนหลัง 24 ตัวอักษรแบบดิบ

    ประโยคนัดหมายจริงมีคำว่า ห้อง/ชั้น/ซอย/ถนน อยู่ในหน้าต่างเสมอ
    """

    @pytest.mark.parametrize(
        "text,expected",
        [
            ("นัดประชุมที่ห้องใหญ่ วันที่ 12/5/45", "พฤษภาคม"),
            ("เจอกันที่ซอยอารีย์ วันที่ 3/4/68", "เมษายน"),
            ("ประชุมที่ชั้น 5 วันที่ 12/5/2545", "พฤษภาคม"),
            ("บ้านอยู่ถนนสุขุมวิท วันที่ 9/9/68", "กันยายน"),
        ],
    )
    def test_มีคำบอกวันที่ติดตัวเลขต้องอ่านเป็นวันที่(self, text, expected):
        assert expected in clean_for_speech(text), clean_for_speech(text)

    @pytest.mark.parametrize(
        "text", ["บ้านเลขที่ 12/5/45", "ห้อง 3/4/12", "ที่อยู่ 9/4/76"]
    )
    def test_เลขที่บ้านต้องยังอ่านว่าทับ(self, text):
        assert "ทับ" in clean_for_speech(text)


class Testหน่วยที่ตามหลังชี้ขาดระหว่างช่วงกับสกอร์:
    """สกอร์การแข่งขันไม่มีลักษณนามต่อท้าย ช่วงปริมาณมีเสมอ

    เชื่อถือได้กว่าคำที่อยู่ก่อนเลข เพราะ "อยู่ที่" ใช้ได้ทั้งสองความหมาย
    """

    @pytest.mark.parametrize(
        "text",
        [
            "คะแนนสอบอยู่ที่ 80-90 คะแนนครับ",
            "ช่วงคะแนน 70-80 ถือว่าดีครับ",
            "คะแนนช่วง 85-90 ครับ",
            "ได้คะแนนสอบระหว่าง 85-90 คะแนน",
        ],
    )
    def test_มีหน่วยตามหลังต้องอ่านว่าถึง(self, text):
        got = clean_for_speech(text)
        assert "ถึง" in got and "ต่อ" not in got, got

    @pytest.mark.parametrize(
        "text", ["สกอร์ตอนนี้อยู่ที่ 2-1", "ทีมชนะ 3-1", "ผลบอล 2-1"]
    )
...
class Testบริบทสกอร์ต้องไม่กลืนช่วงปริมาณ:
    """ชนะ/แพ้/เสมอ เป็นคำไทยธรรมดา การให้ชนะหน่วยทุกชนิดพลาดมากกว่าที่แก้ได้"""

    @pytest.mark.parametrize(
        "text",
        [
            "คนที่ชนะได้ 80-90 คะแนน",
            "ทีมที่ชนะจะได้เงินรางวัล 5000-10000 บาท",
            "ผู้ชนะเลิศได้เงิน 50000-100000 บาท",
            "คนแพ้อาหารทะเลมี 10-15 เปอร์เซ็นต์",
            "ถ้าอยากชนะต้องซ้อมวันละ 2-3 ชั่วโมง",
            "ทีมที่แพ้ต้องจ่ายค่าปรับ 1000-2000 บาท",
            "การแข่งเสมอกันบ่อย ปีนี้ 5-8 ครั้ง",
            # คำสกอร์ติดตัวเลขพอดี แต่หน่วยที่ตามหลังไม่ใช่หน่วยสกอร์ —
            # "เสมอ 5-8 ครั้ง" คือจำนวนครั้งที่เสมอ ไม่ใช่ผลการแข่ง
            "ปีนี้เสมอ 5-8 ครั้ง",
            "เขาแพ้ 3-5 ครั้ง",
        ],
    )
    def test_ช่วงปริมาณต้องอ่านว่าถึง(self, text):
        got = clean_for_speech(text)
        assert "ถึง" in got and "ต่อ" not in got, got
    ...
    def test_สกอร์จริงต้องอ่านว่าต่อ(self, text):
        got = clean_for_speech(text)
        assert "ต่อ" in got and "ถึง" not in got, got


class Testคำว่าช่วงต้องดูสิ่งที่ตามหลัง:
    """บัญชี "ห้าม" คำบอกเวลาไม่มีวันครบ — กลับด้านเป็นดูคำเชิงปริมาณแทน"""
    ...
    @pytest.mark.parametrize(
        ...
        [
            "ช่วงนี้สกอร์ออกมา 2-1 ตลอด",
            "ช่วงต้นเกมชนะ 2-0",
            "ช่วงเวลานั้นสกอร์ 1-0",
            "ช่วงพักครึ่งสกอร์ 1-1",
            "ช่วงวันหยุดทีมชนะ 4-2",
            "ทีมนี้ช่วงหลังชนะ 3-0 บ่อยมาก",
        ],
    )
    def test_ช่วงที่บอกเวลาต้องไม่ทำให้สกอร์อ่านผิด(self, text):
    ...
    def test_ช่วงเชิงปริมาณต้องอ่านว่าถึง(self, text):
        got = clean_for_speech(text)
        assert "ถึง" in got and "ต่อ" not in got, got
