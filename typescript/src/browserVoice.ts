    ...
    const ready = (async () => {
      ...
      connection = this.client.stream({
        ...
        onClose: () => {
          ...
          // ยามนี้ยังไม่มีเทสต์คุ้ม — ถอดออกแล้วชุดทดสอบยังเขียว
          //
          // ลองเขียนเทสต์แล้วสร้างฉากที่แยกมันออกมาไม่ได้ด้วยของปลอมชุดปัจจุบัน
          // (ต้องให้ socket ที่ล้มยิง close มาถึง *หลัง* ตัวใหม่เปิดใช้งานแล้ว
          // พอดี) จึงบันทึกไว้ตรงนี้แทนการใส่เทสต์ที่ผ่านทั้งที่ยามถูกถอด
          // ซึ่งจะหลอกว่ามีความคุ้มครองทั้งที่ไม่มี
