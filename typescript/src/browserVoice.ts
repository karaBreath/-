...
export class VoiceConversation {
  ...
  /** เพิ่มขึ้นทุกครั้งที่กดหยุด — ใช้ยกเลิก start() ที่เข้าคิวไว้ก่อนหน้า */
  private stopEpoch = 0;
  ...
  start(): Promise<void> {
    ...
    // จับ "รุ่นของการกดหยุด" ไว้ตอน *เข้าคิว* ไม่ใช่ตอนถึงคิว
    //
    // stop() ปิดธง running ทันทีแบบซิงโครนัส แต่ cleanup() ยังต่อท้ายคิว
    // ถ้ามี start() เข้าคิวไว้ก่อน stop() ลำดับในคิวจะเป็น beginListening
    // แล้วค่อย cleanup ตัว beginListening นั้นจึงตื่นมาเห็น running เป็น false
    // ทั้งที่ตัวอัดเสียงและตัวถอดเสียงของเซสชันเดิมยังมีชีวิตอยู่ แล้วเขียนทับ
    // ทั้งคู่โดยไม่ปิดของเก่า — ของเก่ากลายเป็นผีที่ยังฟังเสียงผู้ใช้อยู่
    const epoch = this.stopEpoch;
    const guarded = async (): Promise<void> => {
      // มีคนกดหยุดหลังจากเราเข้าคิวไปแล้ว — คำสั่งเริ่มนี้ล้าสมัยไปแล้ว
      if (this.stopEpoch !== epoch) return;
      await this.beginListening();
    };
    const run = this.lifecycle.then(guarded, guarded);
  ...
  private async beginListening(): Promise<void> {
    // เกราะชั้นสุดท้าย: ห้ามเขียนทับตัวอัดเสียง/ตัวถอดเสียงที่ยังมีชีวิตอยู่
    // ไม่ว่าธง running จะบอกอะไร ของที่ถูกเขียนทับไม่มีใครปิดได้อีกเลย
    if (this.running || this.recorder || this.recognition) return;
  ...
  interrupt(): void {
    // ถือสำเนาของ "เปลี่ยนเป็นกำลังฟัง + เปิดไมค์กลับมา" เป็นของตัวเอง
    // ไม่เดินผ่าน finishTurn ยามที่นั่นจึงคุ้มไม่ถึง ต้องมีของตัวเอง
    //
    // onspeechstart ที่ยังค้างอยู่ยิง interrupt() ได้หลังผู้ใช้กดหยุด
    // แล้วป้ายบนหน้าเว็บค้างที่ "กำลังฟัง" ตลอดไป ทั้งที่ไมค์ปิด socket ปิด
    // และ recognizer ถูก abort ไปแล้ว
  ...
  /** หยุดบทสนทนาทั้งหมด */
  stop(): void {
    // เลื่อนรุ่นทันทีแบบซิงโครนัส *ก่อน* เข้าคิว
    //
    // การเข้าคิวอย่างเดียวแก้เรื่อง start แซง stop ได้ แต่ทำให้ยามที่
    // beginListening ใช้เช็คว่า "ผู้ใช้กดหยุดระหว่างรอสิทธิ์ไมโครโฟนหรือยัง"
    // กลายเป็นโค้ดตาย เพราะรุ่นไม่ถูกเลื่อนจนกว่า cleanup จะได้คิว ซึ่งเป็น
    // *หลัง* beginListening ทำงานเสร็จแล้ว ผลคือกดหยุดแล้วไมโครโฟนยังเปิดขึ้นมา
    // UI ขึ้นว่า "กำลังฟัง" แล้วค่อยปิด — ผู้ใช้เห็นไฟไมค์ติดหลังกดหยุด
    // ต้องใช้ทั้งสองกลไก ไม่ใช่เลือกอย่างเดียว
    this.stopEpoch += 1;
    this.generation += 1;
    // ปิดธง "กำลังทำงาน" ทันทีด้วย ไม่ใช่ปล่อยให้ cleanup ทำตอนได้คิว
    //
    // ยามทุกตัวที่อ้าง running (finishTurn, onend ของ recognizer, interrupt)
    // ตาบอดตลอดช่วงคาบเกี่ยวถ้าไม่ปิดตรงนี้ — recognizer ถูกสั่ง start ใหม่
    // หลังผู้ใช้กดหยุดแล้ว ไฟไมค์ติดกลับมาชั่วขณะก่อน cleanup จะตามมา abort
    this.running = false;
  ...
  private async cleanup(): Promise<void> {
    ...
    if (recognition) {
      ...
      // ต้องปลด onspeechstart ด้วย — มันคือตัวที่เรียก interrupt() ในโหมด
      // พูดแทรก ถ้าปล่อยไว้ เสียงที่ recognizer ยังประมวลผลค้างอยู่จะปลุก
      // บทสนทนาที่ปิดไปแล้วขึ้นมาใหม่
  ...
  pushToTalkStart(): void {
    // หยุดไปแล้วต้องไม่เข้าโหมดฟัง — ไม่งั้นสถานะค้างที่ "กำลังฟัง" ถาวร
    if (!this.running) return;
  ...
  async pushToTalkStop(): Promise<void> {
    // หยุดไปแล้วต้องไม่อัปโหลดเสียงต่อ
    //
