...
#
# "ไม่" เดี่ยว ๆ ต้องอยู่ด้วย — "ไม่ลบความจำนะ" เคยถูกอ่านเป็นคำสั่งลบ
# เช่นเดียวกับ "ค่อย" (เอาไว้ทีหลัง) และ "เคย" (เล่าเรื่องอดีต ไม่ใช่สั่ง)
_NEGATOR_BEFORE = re.compile(
    r"(?:อย่าเพิ่ง|อย่า|ไม่ต้อง|ไม่อยาก|ไม่|ห้าม|ยังไม่|ค่อย|เคย)\s*$"
)
...
def _pending_summary(stats: "SpeakerStats") -> str:
    ...
    if stats.turns > 0:
        # ไม่บอกจำนวนข้อความ — บอกแล้วขัดกับประโยคตอนลบจริงเสมอ
        #
        # ตัวเลขนี้อ่านจากสถิติ *ก่อน* เทิร์นคำขอกับคำตอบจะถูกบันทึกลงไป
        # ผู้ใช้จึงได้ยิน "อีกหนึ่งข้อความ" ตอนถาม แล้ว "3 ข้อความ" ตอนลบจริง
    ...
    * ``False`` — **จำเป็นสำหรับเซิร์ฟเวอร์ที่ใช้ session ร่วมกัน** ไม่งั้นคำขอของ
    ...
    def _handle_memory_command(
        ...
        if speaker is not None and speaker.id in self._pending_forget:
            ...
            #
            # เคยมีสาขาตรงนี้ที่ถือว่า "ประโยคที่ขึ้นต้นด้วยคำตอบรับ *และ* มีคำขอลบ
            # อยู่ด้วย" คือการยืนยัน โดยให้เหตุผลว่ามีหลักฐานสองชั้น — ผิด และ
            # อันตรายมาก ทั้งสองตัวตรวจเป็นแบบ "มีคำนี้อยู่ที่ไหนสักแห่ง" ไม่ใช่
            # โครงสร้างประโยค ประโยคที่ผู้ใช้ *ห้าม* ลบจึงเข้าเงื่อนไขครบ:
            #
            #   "ยืนยันว่าไม่ลบความจำนะ"        -> ลบทิ้งทั้งหมด
            #   "แน่ใจว่าไม่อยากลบความจำครับ"   -> ลบทิ้งทั้งหมด
            #   "ใช่ เมื่อวานผมลบความจำในมือถือไป" -> ลบทิ้งทั้งหมด
            #
            # ด่านนี้เป็นด่านสุดท้ายก่อนลบข้อมูลถาวรที่กู้คืนไม่ได้ หลักการของมัน
            # คือ "เข้มงวดจนยอมพลาด" การพลาดแปลว่าผู้ใช้ต้องพูดว่า "ยืนยัน"
            # อีกครั้ง ซึ่งบอทบอกให้เขาพูดอยู่แล้วทุกครั้ง ส่วนการพลาดอีกทางคือ
            # ข้อมูลหายถาวรทั้งที่เขาสั่งห้าม
            if answer is None and detect_forget_all(transcript):
                self._pending_forget[speaker.id] = time.time()
                # ย้ำคำขอ = เริ่มนับใหม่ทั้งนาฬิกาและโควตาการเตือน ไม่งั้นคนที่
                # ย้ำจะได้ผ่อนผันน้อยกว่าคนที่ขอครั้งเดียว ซึ่งกลับหัวกลับหาง
