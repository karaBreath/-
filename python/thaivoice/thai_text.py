...
_ASCII_UNITS = {
    ...
    "mm": "มิลลิเมตร",
    "kg": "กิโลกรัม",
    # หน่วยบนฉลากยาและสูตรอาหาร พบพอ ๆ กับรูปภาษาไทย
...
_THAI_UNIT_ABBR = {
    ...
    "ดล.": "เดซิลิตร", "กรัม.": "กรัม", "ซล.": "เซนติลิตร",
...
_PER_UNITS = (
    ...
    "เม็ด", "มื้อ", "แคลอรี", "ไร่", "ตัน", "ล้าน", "หมื่น", "แสน",
    # "ที่" ปลอดภัยแล้วเพราะกฎบังคับให้มีจำนวนนำหน้า — "ระบุสถานที่/เวลา"
    # ไม่มีตัวเลขจึงไม่เข้าเงื่อนไข ส่วน "นั่งได้ 20 ที่/คัน" มี
    "ที่",
...
#
# "ต่อ" ในภาษาไทยแปลว่าอัตราส่วน ต้องมีจำนวนถึงจะมีความหมาย ถ้าไม่บังคับ
# กฎนี้จะยิงใส่คำนามธรรมดาที่บังเอิญลงท้ายด้วยคำในลิสต์ แล้ว "กรอกวัน/เดือน/
# ปีเกิด" ซึ่งเป็นสำนวนไทยที่พบทุกวัน กลายเป็น "วันต่อเดือนต่อปีเกิด"
# ตรงนั้น "/" แปลว่า "หรือ" ไม่ใช่ "ต่อ"
...
_TIME_RANGE_CONTEXT = re.compile(
    r"(?:เวลา|ตอน|นัด|เจอกัน|ประชุม|เริ่ม|เลิก|ออก|เปิด|ปิด|สอบ|ทำการ|ทำงาน|"
    r"ตั้งแต่|บ่าย|เช้า|เย็น|ค่ำ|เที่ยง|ดึก|โมง|ทุ่ม|ตี|รอบ|กะ|เวร)"
)
...
_QUANTITY_UNITS_WEAK = (
    ...
    # "เดือนหน้า" "สัปดาห์หน้า" "นาทีเดียว" พบบ่อยกว่าการใช้เป็นลักษณนามมาก
    "เดือน", "สัปดาห์", "นาที", "ที่นั่ง", "เม็ด", "มื้อ", "แคลอรี",
...
# คำที่บอกว่าเป็นสกอร์การแข่งขันแน่นอน — ต่างจาก "คะแนน" ที่ใช้กับการสอบด้วย
# ต้องอยู่ *ติด* ตัวเลข (ยอมให้มีคำเชื่อมสั้น ๆ คั่นได้)
#
# สกอร์จริงวางคำกริยาไว้ชิดตัวเลข ("บาสชนะ 98-95") ส่วนประโยคที่บอกช่วง
# ปริมาณมักมีคำอื่นคั่น ("คนที่ชนะได้ 80-90 คะแนน" = ผู้ชนะได้คะแนนในช่วงนั้น)
_STRONG_SCORE_CONTEXT = re.compile(
    r"(?:สกอร์|ผลบอล|ผลการแข่ง|ผลแข่ง|ชนะ|แพ้|เสมอ|นำอยู่|ตามอยู่)"
    r"(?:กัน|ไป|อยู่)?\s*$"
)
# หน่วยที่ "เป็นสกอร์ได้" — บริบทสกอร์ชนะหน่วยที่ตามหลังได้เฉพาะหน่วยกลุ่มนี้
#
# ชนะ/แพ้/เสมอ เป็นคำไทยธรรมดา ("คนที่ชนะได้ 80-90 คะแนน", "คนแพ้อาหารทะเล
# มี 10-15 เปอร์เซ็นต์") การให้บริบทสกอร์ชนะหน่วย *ทุกชนิด* ทำให้ช่วงปริมาณ
# จริงถูกอ่านเป็นสกอร์หมด — วัดแล้วพลาดมากกว่าที่แก้ได้เจ็ดเท่า
_SCORE_UNIT = re.compile(r"^\s*(?:คะแนน|แต้ม|ประตู|เซต|ลูก|เกม)")
# คำที่บอกว่าเลขคู่นี้เป็น "ช่วง" ไม่ใช่สกอร์การแข่งขัน
#
...
# "ช่วง" นับเป็นคำบอกช่วง *ยกเว้น* เมื่อตามด้วยคำบอกเวลา
#
# "ช่วงนี้/ช่วงหลัง/ช่วงเช้า" เป็นคำบอกเวลาที่ใช้ทั่วไป การนับมันเป็นคำบอกช่วง
# ทำให้ "ช่วงนี้สกอร์ออกมา 2-1 ตลอด" อ่านว่า "สองถึงหนึ่ง" ซึ่งเปลี่ยนความหมาย
# ส่วน "ช่วงคะแนน 70-80" เป็นคำบอกช่วงจริง
# "ช่วง" นับเป็นคำบอกช่วงก็ต่อเมื่อ *ตามด้วย* คำเชิงปริมาณหรือตัวเลข
#
# กลับด้านจากการไล่แบล็กลิสต์คำบอกเวลา ("ช่วงนี้/ช่วงหลัง/ช่วงต้นเกม/
# ช่วงพักครึ่ง/ช่วงวันหยุด/ช่วงเวลานั้น") ซึ่งไม่มีวันครบ — คำบอกเวลาที่ตาม
# "ช่วง" ได้มีนับไม่ถ้วน ส่วนคำเชิงปริมาณที่ตามได้มีไม่กี่คำ
_RANGE_CONTEXT = re.compile(
    r"(?:ระหว่าง|ตั้งแต่|ประมาณ|ราว|เฉลี่ย|ราคา|อายุ"
    # "ช่วง" ที่อยู่ติดตัวเลขพอดี ("คะแนนช่วง 85-90") — ตัวเลขอยู่นอกหน้าต่าง
    # ที่ตรวจ lookahead จึงมองไม่เห็น ต้องรับกรณีนี้แยก
    r"|ช่วง\s*$"
    r"|ช่วง(?=คะแนน|แต้ม|อายุ|ราคา|เงิน|น้ำหนัก|ความสูง|ส่วนสูง|ตัวเลข|จำนวน))"
)
...
# คำนำหน้าที่บอกว่าเลขคู่นี้เป็น "ลำดับที่ X จากทั้งหมด Y" ไม่ใช่เศษส่วน
_ORDINAL_CONTEXT = re.compile(
    r"(?:งวดที่|ข้อ|หน้า|ตอนที่|รอบที่|ครั้งที่|ชิ้นที่|อันดับ|ลำดับที่|บทที่)"
)
...
def expand_numbers_for_speech(text: str) -> str:
    ...
    * เลขที่ติดกับตัวอักษรหรือเป็นส่วนของ IP -> ปล่อยไว้ ไม่แตะ
    ...
    def range_or_score(match: re.Match[str]) -> str:
        ...
        # บริบทสกอร์ที่ชัดเจน (ชนะ/แพ้/เสมอ/ผลบอล/สกอร์) ชนะหน่วยที่ตามหลัง
        #
        # "สกอร์ไม่มีลักษณนามต่อท้าย" ไม่จริงกับกีฬาที่นับเป็นคะแนน —
        # "บาสชนะ 98-95 คะแนน" มีทั้งคำบอกสกอร์และหน่วย ให้คำบอกสกอร์ชี้ขาด
        # ส่วนหน่วยที่ตามหลังใช้ตัดสินเฉพาะตอนที่บริบทเป็นคำว่า "คะแนน" เปล่า ๆ
        # ซึ่งกำกวมจริง ("คะแนนสอบอยู่ที่ 80-90 คะแนน")
        after = text[match.end() :]
        strong_score = _STRONG_SCORE_CONTEXT.search(window) and (
            not _QUANTITY_UNIT.match(after) or _SCORE_UNIT.match(after)
        )
        scored = strong_score or (
            _SCORE_CONTEXT.search(window)
            and not _QUANTITY_UNIT.match(after)
        )
        if _RANGE_CONTEXT.search(window):
            scored = False
        ...
        # ทีละตัว เพราะปกติมันคือเบอร์โทร
        def side(token: str) -> str:
            trimmed = token.lstrip("0")
            if trimmed and len(token) <= 2:
                return _speak_quantity(trimmed)
            return _speak_quantity(token)

        return f"{side(match.group(1))}{joiner}{side(match.group(2))}"
    ...
    def slash_pair(match: re.Match[str]) -> str:
        ...
        # "เทอม 1/2567" "ไตรมาส 3/2567" "ประกาศฉบับที่ 3/2567" — ตัวล่างเป็นปี
        # พ.ศ. ซึ่งเป็นรูปแบบเลขที่เอกสารที่พบบ่อยที่สุดรูปหนึ่งในภาษาไทย
        if 2400 <= bottom <= 2700 and len(match.group(2)) == 4:
            return f"{left}ทับ{right}"
        # "งวดที่ 1/12" "หน้า 1/20" "ตอนที่ 1/8" คือลำดับที่ ไม่ใช่เศษส่วน
        if _ORDINAL_CONTEXT.search(window):
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        # หน่วยที่ตามหลังเป็นหลักฐานที่หนักกว่ากฎ "ยาวเกินแปดหลักให้อ่านทีละตัว"
        # "ยอดขาย 12345678 บาท" เป็นจำนวนเงิน ไม่ใช่รหัส
        if (
            not decimal
            and len(whole) >= _DIGIT_BY_DIGIT_LENGTH
            and len(whole) <= _MAX_INT_DIGITS
            and not whole.startswith("0")
            and _QUANTITY_UNIT.match(text[match.end() :])
        ):
            prefix = {"-": "ลบ", "+": "บวก"}.get(sign, "")
            return prefix + thai_number_to_words(int(whole))
...
def _abbreviation_stems(abbreviations: "Iterable[str]") -> set[str]:
    """ทุกส่วนนำหน้าจุดของตัวย่อ — "ลบ.ซม." ให้ทั้ง "ลบ" และ "ลบ.ซม"

    ต้องสร้างจากตารางจริง ไม่ใช่ไล่พิมพ์มือ ไม่งั้นรอบหน้าที่เพิ่มหน่วยใหม่
    ก็จะพังซ้ำที่เดิมโดยไม่มีใครรู้
    """
    stems: set[str] = set()
...
# ตัวย่อที่จุดของมันไม่ใช่จุดจบประโยค
#
# เดือนและหน่วยต้องอยู่ด้วย ไม่ใช่แค่ยศกับคำนำหน้าชื่อ — "5 เม.ย. 2569" เคยถูก
# ตัดเป็น "5 เม." + "ย. 2569" แล้วชื่อเดือนหายทั้งคำ TTS อ่าน "เมอ" ส่วน
# "120 ตร.ม." กลายเป็น "ตอรอ" + "มอ" พังเฉพาะตัวย่อที่มีพยัญชนะสองตัวหน้าจุด
# ("มี.ค." "ก.พ." รอดเพราะตัวหน้าจุดเป็นสระหรือพยัญชนะตัวเดียว) จึงจับได้ยาก
_THAI_ABBREVIATIONS = tuple(
    sorted(
        {
            # "พล" ต้องอยู่ด้วย ไม่ใช่แค่ "พล.อ" — รายการที่มีจุดอยู่ในตัวกันได้
            # แค่จุด *ที่สอง* จุดแรกของ "พล.อ. ประยุทธ์" จึงยังตัดกลางยศอยู่
            "ผศ", "รศ", "ดร", "นพ", "พญ", "ทพ", "บมจ", "บจก", "หจก", "ปตท",
            "กฟผ", "รร", "รพ", "พล", "ร.ต", "ร.ท", "ร.อ", "พล.ต.อ", "พล.ต.ท",
            "พ.ต", "พ.ท", "พ.อ", "พล.ต", "พล.ท", "พล.อ", "น.ส", "ด.ช", "ด.ญ",
        }
        | _abbreviation_stems(_MONTH_ABBR)
        | _abbreviation_stems(_THAI_UNIT_ABBR)
        | _abbreviation_stems(_ERA_ABBR)
    )
)
