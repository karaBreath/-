...
# การปฏิเสธแบบขึ้นต้นเลยทำให้คนกลุ่มนี้แนะนำตัวไม่ได้เลย แต่ถ้าคำที่จับได้
# ยาวกว่าคำนำมาก ("เพลงนี้เพราะมา") แปลว่าจับเลยขอบชื่อไปแล้วจริง ๆ
_AMBIGUOUS_NAME_PREFIXES = (
    "แมว", "หมา", "เพลง", "ลูก", "ยา", "ร้าน", "ทีม", "วง", "เรื่อง",
    "ถนน", "ซอย", "เมนู", "โรค", "หนัง", "เพื่อน", "เขา", "เธอ",
)
_AMBIGUOUS_OVERSHOOT = 4
...
def _clean_name(raw: str) -> str | None:
    ...
    if any(name.startswith(prefix) for prefix in _NOT_A_NAME_PREFIXES):
        return None
    if any(
        name.startswith(prefix) and len(name) - len(prefix) >= _AMBIGUOUS_OVERSHOOT
