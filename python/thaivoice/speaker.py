...
_NAME_RULES = [
    ...
    # ต้องมีขอบซ้ายเหมือนกฎอื่นทุกข้อ — "เรียกว่า" เป็นสำนวนอธิบายที่ใช้บ่อย
    # ที่สุดสำนวนหนึ่ง ถ้าไม่ยึดต้นวรรค ประโยคอธิบายธรรมดาจะแย่งตัวตนผู้ใช้ทันที
    # ("ยาตัวนี้เรียกว่าพาราเซตามอลครับ" -> ตัวตนใหม่ชื่อ "พาราเซตามอล")
    # ตัวอย่างในคอมเมนต์เดิม ("โรคนี้เรียกว่าโรคเบาหวาน") รอดเพราะบังเอิญ
    # "โรค" อยู่ในบัญชีคำกำกวมเท่านั้น คำนามอื่นทะลุหมด
    _NameRule(re.compile(rf"(?:^|\s)เรียกว่า\s*({_NAME_CHARS}?)\s*ก็ได้")),
    _NameRule(re.compile(rf"(?:^|\s)เรียกว่า\s*({_NAME_CHARS})")),
    #
...
# ลำดับสำคัญ: ต้องลอง "เรียกว่า" ก่อน "เรียก" ไม่งั้น "เรียกว่าลูกปัดก็ได้ค่ะ"
# จะเหลือ "ว่าลูกปัด"
_LEADING_CALL_VERBS = ("เรียกว่า", "เรียก")

# คำที่บอกว่าประโยคนี้เป็น *คำขอ* ไม่ใช่การบอกชื่อ
#
# ใช้กับสิ่งที่เหลือหลังตัดคำกริยา "เรียก" ออกในทางถอย ("เรียกช่างมาหน่อย")
# คำพวกนี้ไม่เคยเป็นส่วนของชื่อคนไทย และเป็นเครื่องหมายของประโยคขอร้อง
# ตั้งใจไม่ใส่คำสั้นที่เป็นพยางค์ของชื่อไทยได้ — "ที" "ให้" "ด้วย" "ก่อน"
#
# การเทียบแบบสตริงย่อยทำให้ชื่อจริงถูกปฏิเสธ: ประทีป ทีปกร ทีน่า มาร์ที
# คนที่ชื่อประทีปลงทะเบียนด้วยรูปประโยคนี้ไม่ได้เลย บอทถามชื่อแล้วถามซ้ำวนไป
# นี่เป็นความผิดพลาดแบบเดียวกับที่เคยเกิดตอนใส่ "มา" ลงบัญชีคำต้องห้าม
_REQUEST_WORDS = ("หน่อย", "ด่วน", "ช่วย", "เดี๋ยวนี้", "ตอนนี้")

# คำลงท้ายสั่งการ — ต้องเทียบที่ *ท้ายประโยค* ไม่ใช่แบบสตริงย่อย
#
# รอบก่อนใส่ "ซิ"/"สิ" ลงบัญชีข้างบน ซึ่งเทียบแบบสตริงย่อย แล้วกินชื่อไทยจริง
# ทันที: สิริ ประสิทธิ์ สิงห์ สิรินทร์ สิทธิ์ ซินดี้ — ความผิดพลาดแบบเดียวกับ
# ที่คอมเมนต์ข้างบนเพิ่งเตือนไว้เอง ("สิ" พบในชื่อไทยบ่อยกว่า "ที" เสียอีก)
# พอเทียบที่ท้ายประโยค "ที" ก็กลับมาใช้ได้อย่างปลอดภัยด้วย
_REQUEST_TAILS = ("ซิ", "สิ", "เถอะ", "ที")


def _is_request_phrase(text: str) -> bool:
    """ข้อความนี้เป็นคำขอ ไม่ใช่ชื่อ"""
    body = text.strip(" .,!?\"'")
    for suffix in _NAME_SUFFIXES:
        if body.endswith(suffix):
            body = body[: -len(suffix)].strip()
            break
    if any(word in body for word in _REQUEST_WORDS):
        return True
    return body.endswith(_REQUEST_TAILS)
    ...
    if expecting_name:
        ...
        if len(stripped) <= 24 and len(stripped.split()) <= 2:
            ...
            for verb in _LEADING_CALL_VERBS:
                if not body.startswith(verb) or len(body) - len(verb) < 2:
                    continue
                rest = body[len(verb) :]
                # ตัดคำกริยาทิ้งได้ก็ต่อเมื่อสิ่งที่เหลือ *เป็นชื่อได้จริง*
                #
                # การตัดทิ้งเฉย ๆ ทำให้ประโยคที่ยาวเกินเพดานหดลงมาผ่านด่านได้
                # "เรียกช่างมาหน่อยครับ" กลายเป็นตัวตนชื่อ "ช่างมาหน่อย"
                # "เรียกแท็กซี่ให้หน่อย" กลายเป็น "แท็กซี่ให้หน่อย"
                # ทั้งคู่เป็นคำสั่งธรรมดาที่คนพูดใส่ผู้ช่วยเสียงตลอดเวลา
                # ไม่ใช่ทุกคำตอบหลังคำถาม "ชื่ออะไร" จะเป็นชื่อ
                if _is_request_phrase(rest):
                    return None
                body = rest
                break
