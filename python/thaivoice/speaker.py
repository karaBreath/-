...
# ลำดับของกฎที่ *มี* สรรพนามบุรุษที่หนึ่งยืนยันว่าเป็นการแนะนำตัวจริง
#
# เฉพาะกฎเหล่านี้เท่านั้นที่ผ่อนการกันคำกำกวมได้ ("ดิฉันชื่อลูกปัดค่ะ"
# เป็นชื่อคนแน่นอน) ส่วนกฎที่ไม่มีสรรพนามต้องเข้ม เพราะ "เรียกว่า" เป็น
# สำนวนอธิบายที่ใช้บ่อยที่สุดสำนวนหนึ่งในภาษาไทย
# ("โรคนี้เรียกว่าโรคเบาหวานครับ" ไม่ใช่การบอกชื่อตัวเอง)
#
# กฎที่ 0 ("ชื่อเล่นชื่อX") และกฎที่ 7 ("เรียก X ก็ได้") เป็นการแนะนำตัวชัดเจน
# พอ ๆ กับรูปที่มีสรรพนาม ไม่มีความหมายเชิงอธิบายเหมือน "เรียกว่า" ลอย ๆ
# ตอนเพิ่มกฎจาก 8 เป็น 10 ข้อ ดัชนีชุดนี้ไม่ได้ถูกปรับตาม กฎที่ 0 จึงกลายเป็น
# แบบเข้ม แล้วชื่อเล่นอย่าง "ลูกปัด" "แมวเหมียว" ถูกปฏิเสธหมด
#
# แต่การปลดด่านคำกำกวมอย่างเดียวหลวมเกินไป — "ชื่อเล่นลูกชายผมครับ" กลายเป็น
# ชื่อ "ลูกชายผม" และ "เรียกเพื่อนมาก็ได้ครับ" กลายเป็นชื่อ "เพื่อนมา"
# จึงต้องมีด่านสองอันที่แคบกว่าและตรงจุดกว่ามาแทน (ดู _NOT_A_NAME_ENDINGS
# และ _SUMMON_VERBS ข้างล่าง) ทั้งสองอันจับสิ่งที่ด่านคำกำกวมเดิมพลาดอยู่แล้ว
# ด้วย — "เรียกช่างมาก็ได้ครับ" เคยกลายเป็นชื่อ "ช่างมา" มาตั้งแต่ก่อนหน้านี้
_PRONOUN_CONFIRMED_PATTERNS = frozenset({0, 1, 3, 4, 7, 8})

# ดัชนีของกฎที่รูปประโยคชนกับสำนวน "เรียก <คน> มา" (สั่งให้ใครมา)
# "เรียกลูกปัดก็ได้ค่ะ" เป็นการบอกชื่อ แต่ "เรียกช่างมาก็ได้ครับ" ไม่ใช่
# ทั้งสองประโยคเข้ากฎเดียวกัน แยกได้ที่คำกริยาบอกทิศทางที่โผล่ในตัวชื่อ
_SUMMON_PRONE_PATTERNS = frozenset({7})
...
_NAME_PATTERNS = [
    ...
    #
    # แยกรูปที่ *มี* สรรพนามออกจากรูปที่ไม่มี เพราะ "เรียกว่า" ลอย ๆ เป็นสำนวน
    # อธิบายที่ใช้บ่อยมาก ("โรคนี้เรียกว่าโรคเบาหวาน" ไม่ใช่การบอกชื่อตัวเอง)
    # รูปที่มีสรรพนามเท่านั้นที่ยืนยันได้ว่าเป็นการแนะนำตัว
    ...
    re.compile(rf"เรียกว่า\s*({_NAME_CHARS}?)\s*ก็ได้"),
    re.compile(rf"เรียกว่า\s*({_NAME_CHARS})"),
    # "เรียกเดชก็ได้"
...
# กริยาบอกทิศทางในสำนวน "เรียก <คน> มา" — ถ้าโผล่กลางชื่อที่จับได้จากกฎที่ 7
# แปลว่าประโยคนั้นคือการสั่งให้ใครมา ไม่ใช่การบอกชื่อ
#
# ตรวจเฉพาะตำแหน่งที่ *ไม่ใช่* ต้นคำ เพราะชื่อไทยจริงขึ้นต้นด้วยพยางค์นี้ได้
# (มาลี มานี มาริษา ไปรยา) ส่วนชื่อที่ *ลงท้าย* ด้วย "มา" (อุมา ปัทมา) จะถูก
# ปฏิเสธไปด้วยเมื่อพูดผ่านกฎที่ 7 — ยอมพลาดตรงนี้ เพราะอีกทางคือผูกลายเสียง
# ของผู้ใช้เข้ากับตัวตนปลอมชื่อ "ช่างมา" ถาวร และรูปอื่น ("หนูชื่ออุมาค่ะ")
# ยังใช้ได้ตามปกติ
_SUMMON_VERBS = ("มา", "ไป")
...
def _clean_name(
    raw: str, strict_ambiguous: bool = True, reject_summons: bool = False
) -> str | None:
    ...
    ``reject_summons`` ปฏิเสธชื่อที่มีกริยาบอกทิศทางอยู่กลางคำ ใช้กับกฎที่รูป
    ประโยคชนกับสำนวน "เรียก <คน> มา" ("เรียกช่างมาก็ได้ครับ" ไม่ใช่การบอกชื่อ)
    """
    name = raw.strip(" .,!?\"'")

    # ตรวจสำนวน "เรียก <คน> มา" กับข้อความ *ก่อน* ตัดคำนำหน้า ไม่งั้นคำนำหน้าที่
    # ถูกตัดออกจะพาคำกริยาเลื่อนไปอยู่ต้นคำแล้วรอดด่านนี้ไป
    # ("เรียกน้องมาช่วยก็ได้" -> ตัด "น้อง" -> "มาช่วย" -> "มา" อยู่ต้นคำ)
    if reject_summons and any(verb in name[1:] for verb in _SUMMON_VERBS):
        return None
...
def extract_name_claim(text: str, expecting_name: bool = False) -> str | None:
    ...
    for index, pattern in enumerate(_NAME_PATTERNS):
        for match in pattern.finditer(text):
            name = _clean_name(
                match.group(1),
                strict_ambiguous=index not in _PRONOUN_CONFIRMED_PATTERNS,
                reject_summons=index in _SUMMON_PRONE_PATTERNS,
            )
            if name:
                return name
